...
export const ShopeeAffiliateCard: React.FC<ShopeeAffiliateCardProps> = ({
  ...
  const displayTitle =
    title ||
    (language === 'es'
      ? 'Libros Seleccionados de Desarrollo Personal y Carrera'
      : language === 'en'
      ? 'Selected Career & Personal Growth Books'
      : 'Buku Pengembangan Diri & Karir Pilihan (Atomic Habits / Psychology of Money)');

  const displayDescription =
    description ||
    (language === 'es'
      ? 'Libro físico original para maximizar su potencial de personalidad y disciplina laboral.'
      : language === 'en'
      ? 'Original physical book to boost your personality potential and career discipline.'
      : 'Buku fisik original cetakan terbaru untuk melejitkan potensi kepribadian dan disiplin kerja Anda.');

  const badgeLabel =
    language === 'es'
      ? 'RECOMENDACIÓN SHOPEE'
      : language === 'en'
  ...
  const displayBadgeText =
    badgeText ||
    (language === 'es'
      ? 'Tienda Oficial Shopee / Original'
      : language === 'en'
      ? 'Shopee Official Store / Original'
      : 'Shopee Official Store / Original');
    ...
    <div className={`p-4 sm:p-5 rounded-2xl bg-gradient-to-br from-amber-500/10 via-orange-500/5 to-slate-900/5 dark:from-amber-950/40 dark:to-orange-950/20 border border-orange-300/80 dark:border-orange-800/60 shadow-sm transition-all hover:shadow-md ${className}`}>
      ...
      <div className="flex items-center justify-between gap-2 mb-3">
        ...
        <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider flex items-center gap-1">
          <ShieldCheck className="w-3 h-3 text-emerald-500" /> {affiliateLabel}
        </span>
      </div>
